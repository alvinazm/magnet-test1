...
def promo_action(roi):
    """自定阈值:ROI ≥2 保留;1 ≤ ROI <2 调整;ROI <1 取消(已在异常清单披露)。"""
    if roi >= 2:
        return "保留"
    if roi >= 1:
        return "调整"
...
anomalies = [
    ("A-001", "时效性", "旧版门店清单_2024.xlsx", "全部字段",
     ...
     "附件文件名含 _2024;门店数 50 家 vs 当前 42 家;《渠道与门店管理策略.docx》第 5.2 条不得用外部/过期数据"),
    ...
    ("A-005", "口径不清", "2026H1会员与跨渠道订单.xlsx", "跨渠道行为.累计消费 / 跨渠道购买渠道数",
     "会员累计消费以《会员订单》为口径;《渠道销售明细》的会员标识仅用于渠道归因,不重复计入。",
     "会员分层与 Top10% 消费贡献", "低",
    ...
    ("A-007", "数据冲突", "全部 9 个附件", "跨表主键与聚合口径",
     "已对门店ID/渠道ID/订单号/SKU/会员ID/活动ID 六组主键及净收入、O2O 履约贡献、佣金、活动销售额等聚合口径做跨表核验,未发现数值冲突。",
     "全部结论", "低", "无需处理;此项作为跨表一致性核验记录留档",
     "本报告《数据血缘说明》第七节;数据自检脚本 113 项通过"),
    ("A-008", "缺失", "2026H1促销活动记录.xlsx", "活动效果",
...
doc.add_paragraph(
    ...
    f"按《渠道与门店管理策略》第 2 章规则(先按到店坪效定级,租售比 >35% 直接列为 D 级):"
...
doc.add_paragraph(
    ...
    f"({', '.join(store[store['面积是否调整'] == '是'].index)}),其坪效口径可能失真,已在《异常与待核清单》披露。"
)
...
> **数据版本**:num.py v3.1 生成的 `input/` 9 个附件(数据自检 113 项通过)
> **生成时间**:2026-09-23
...
**渠道/活动/SKU/会员主键**:渠道ID(《2026H1渠道销售明细.xlsx》/《2026H1线上投放与平台费用.xlsx》/《2026H1会员与跨渠道订单.xlsx》/《2026H1促销活动记录.xlsx》)、订单号(《2026H1渠道销售明细.xlsx》/《2026H1会员与跨渠道订单.xlsx》)、SKU编码(《2026H1渠道销售明细.xlsx》/《2026H1促销活动记录.xlsx》/《SKU主数据.xlsx》)、会员ID(《2026H1会员与跨渠道订单.xlsx》)、活动ID(《2026H1促销活动记录.xlsx》)。
...
| 面积是否调整 | 《2026H1门店经营台账.xlsx》 门店基础信息.面积是否调整 | 直接读取 | = 是 的门店坪效口径可能失真,已在异常清单披露 |
...
| 数据状态 | 面积是否调整 | = 是 → 待补(坪效口径待确认);否则 已确认 | 待补含义见异常清单 A-002 |
...
| H2资源调整方向 / 建议投入幅度 | 自定规则(见异常清单 A-006 同类披露) | 净利>0 且 费用率≤30% → 增投 +20%;净利<0 且 费用率≥33% → 收缩 −15%;其余 → 维持 | 附件内未规定阈值,已在本说明书与报告中披露 |
...
| 建议动作 | 自定阈值 | ROI ≥2 保留;1 ≤ ROI <2 调整;ROI <1 取消 | 阈值已披露(异常清单),可按管理层门槛重算 |
...
| 自定项 | 取值 | 披露位置 |
| --- | --- | --- |
| 渠道资源调整规则 | 净利>0 且 费用率≤30% → 增投 +20%;净利<0 且 费用率≥33% → 收缩 −15%;其余 → 维持 | 异常清单 A-006 同类口径披露、本说明书第三节 |
| 促销建议动作阈值 | ROI ≥2 保留;1 ≤ ROI <2 调整;ROI <1 取消 | 异常清单 A-006、本说明书第四节 |
...
1. 运行 `.venv/bin/python docs/specs/data-validation.py`,确认 9 个附件通过 113 项校验;
2. 运行 `.venv/bin/python build_solution.py`,可重新生成 `output/` 下全部交付物与本说明书;
3. 任取交付物中的一个金额,按本说明书第二至四节的"数据来源 + 分析逻辑"列即可定位到附件 Sheet 与字段;
4. 运行 `.venv/bin/python docs/specs/validate_lineage.py`,可自动扫描本文档中的每个数值是否能在附件中找到对应或可复算。
