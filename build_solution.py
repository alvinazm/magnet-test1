...
anomalies = [
    ...
    ("A-019", "口径不清", "渠道资源再配置建议.xlsx", "H2资源调整方向 / 建议投入幅度 / 销售费用率",
     "渠道 H2 投入方向与幅度阈值为本报告自定:渠道净利 > 0 且销售费用率 ≤30% → 增投 +20%;渠道净利 < 0 且销售费用率 ≥33% → 收缩 −15%;其余维持。《渠道与门店管理策略.docx》与附件均未规定该阈值,亦无集团已批准的 H2 渠道预算上限。",
...
doc.add_paragraph("")
doc.add_paragraph(
    f"【分析判断】上述方向与幅度阈值为本报告自定规则(A-019):渠道净利 > 0 且销售费用率 ≤30% 增投 +20%,"
    f"渠道净利 < 0 且销售费用率 ≥33% 收缩 −15%,其余维持;《渠道与门店管理策略.docx》第 1.1–1.3 条只给渠道定位,"
    f"未规定阈值,故 H2 金额均为建议值。"
)
doc.add_paragraph(
...
appendix = [
    ...
    ("渠道 H2 投入方向与幅度阈值", "《渠道与门店管理策略.docx》", "第 1.1–1.3 条仅规定渠道定位与平台利润分级;投入阈值为本报告自定,见 A-019"),
