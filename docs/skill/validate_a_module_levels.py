...
用途:自动检查 A 标准的中间状态档位是否:
    1. 显式对应标准描述的关键对象(培训文档 6.3.2 节)
    2. 不含培训文档 6.4 节禁止的抽象词
    3. 不是半截话(不以"但"或";"结尾)

使用:
    .venv/bin/python docs/skill/validate_a_module_levels.py
...
DOC_PATH = WORK / "docs/specs/step3-evaluation-rubric.md"


# ============================================================
# 1. A 标准的"关键对象"清单(23 条,v4.0 评估表)
# ============================================================
# 每个对象必须在档位内容中显式出现
EXPECTED_OBJECTS = {
    "A-1": ["净收入", "毛利", "毛利率", "客单价"],
    "A-2": ["费用合计", "销售费用率", "投放 ROI"],
    "A-3": ["到店坪效", "租售比", "租金及物业费"],
    "A-4": ["A/B/C/D", "到店坪效", "D 级"],
    "A-5": ["到店销售额", "门店 O2O 履约贡献", "快递"],
    "A-6": ["增量销售额", "增量毛利", "ROI"],
    "A-7": ["门店ID", "渠道ID", "订单号", "SKU编码", "会员ID", "活动ID"],
    "A-8": ["异常", "时效性", "口径不清", "缺失", "越界"],
    "A-9": ["文件", "Sheet", "字段", "记录"],
    "A-10": ["实际数", "估算", "建议值", "待核"],
    # A-11: 无档位(二元判断:面积调整三店的披露)
    "A-12": ["租期结束", "闭店违约金", "连续 6 个月", "租售比连续 3 个月"],
    "A-13": ["累计消费", "跨渠道购买渠道数", "会员订单"],
    "A-14": ["履约时效", "适销期", "编制人数", "待补"],
    # A-15: 无档位(二元判断:5 个交付物齐全且命名一致)
    "A-16": ["执行摘要", "附录", "报告期", "封面"],
    "A-17": ["必含字段", "Sheet", "行数"],
    "A-18": ["42 家门店", "6 个渠道", "50 个活动", "2,500"],
    "A-19": ["实际发生额", "建议值", "预算"],
    "A-20": ["货币", "比例", "坪效"],
    "A-21": ["完整文件名", "扩展名", "Sheet", "字段"],
    "A-22": ["章节", "表格", "排版", "附录"],
    "A-23": ["事实", "判断", "建议", "建议值"],
}
...
def main():
    ...
    print("=" * 75)
    print(f"A 模块档位机审(v1.3 - 显式对应 + 抽象词检查)")
